...
<main id="main" class="main" style="height: 80vh">
    <p class="text-2xl font-serif font-bold ">{{__('Edit Cooperative')}}</p><br>
    <form action="{{ route('cooperatives.update', $cooperative) }}" method="POST">
        @csrf
        @method('PUT')
        <label for="name" class="text-xl font-serif font-bold">{{__('Name:')}}</label>
        ...
        <label for="location" class="text-xl font-serif font-bold">{{__('Location')}}</label>
        ...
        <label for="services_offered" class="text-xl font-serif font-bold">{{__('Services Offered:')}}</label>
